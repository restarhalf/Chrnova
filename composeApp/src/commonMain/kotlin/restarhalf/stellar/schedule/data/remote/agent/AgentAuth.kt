...
val AgentAuthPlugin = createClientPlugin("AgentAuthPlugin", ::AgentAuthPluginConfig) {
    ...
    onRequest { request, _ ->
        if (!request.url.encodedPath.startsWith(AGENT_PATH_PREFIX)) return@onRequest
        ...
        telemetry.onIdentityHeaderInjection(true, userId.sha256Prefix())
    }

    on(Send) { request ->
        val call = proceed(request)
        if (!request.url.encodedPath.startsWith(AGENT_PATH_PREFIX)) return@on call
